...
appel                 : IDF liste_arguments {$$= concat_pere_fils(creer_noeud(C_IDF,$1,0),$2);}
                      ;
...
condition             : SI PARENTHESE_OUVRANTE expression PARENTHESE_FERMANTE ALORS ACCOLADE_OUVRANTE liste_instructions ACCOLADE_FERMANTE SINON ACCOLADE_OUVRANTE liste_instructions ACCOLADE_FERMANTE {$$= concat_pere_frere (concat_pere_fils(creer_noeud(C_SI,-979,0),concat_pere_frere($3,$7)),concat_pere_fils(creer_noeud(C_SINON,-976,1),$11));}
                      ;
...
variable              : IDF {$$= creer_noeud(C_IDF,$1,0);}
	 	      | IDF variable_suite {$$=concat_pere_frere(creer_noeud(C_IDF,$1,1),$2);}
		      ;
...
expression_calcul     : expression_calcul PLUS expression_suite {$$=concat_pere_fils(creer_noeud(C_PLUS,-990,1),concat_pere_frere($1,$3));}
                      | expression_calcul MOINS expression_suite {$$=concat_pere_fils(creer_noeud(C_MOINS,-991,0),concat_pere_frere($1,$3));}
		      | expression_suite {$$=$1;}
                      ;

expression_suite      : expression_suite MULT expression_fin {$$=concat_pere_fils(creer_noeud(C_MULT,-989,1),concat_pere_frere($1,$3));}
                      | expression_suite DIV expression_fin {$$=concat_pere_fils(creer_noeud(C_DIV,-988,0),concat_pere_frere($1,$3));}
                      | expression_fin {$$=$1;}
                      ;
